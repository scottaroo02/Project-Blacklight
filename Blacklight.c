...
static Byte inputLen = 0;
static Boolean adminMode = false;
static Byte adminTaps = 0;
...
static void LoadState(void) {
    Word size = sizeof(g);
    SWord v = PrefGetAppPreferences(CREATOR_ID, PREF_ID, &g, &size, true);
...
static void Center(const Char *s, SWord y) {
    SWord w = FntCharsWidth(s, StrLen(s));
    WinDrawChars(s, StrLen(s), (160 - w) / 2, y);
}

static void Button(SWord x, SWord y, SWord w, SWord h, const Char *s) {
    RectangleType r;
    RctSetRectangle(&r, x, y, w, h);
    WinDrawRectangle(&r, 2);
    WinDrawChars(s, StrLen(s),
                 x + (w - FntCharsWidth(s, StrLen(s))) / 2,
                 y + 5);
}
...
static Boolean IsInside(SWord x, SWord y,
                        SWord bx, SWord by,
                        SWord bw, SWord bh) {
...
static Boolean HandlePen(SWord x, SWord y) {
    Byte row;
    Byte col;
    Byte digit = 0;
    ...
    if (g.stage == 0 && IsInside(x, y, 48, 105, 64, 24)) {
        Advance();
        return true;
    }

    if (g.stage == 2 && IsInside(x, y, 52, 125, 56, 22)) {
        Advance();
        return true;
    }

    if (g.stage == 4 && IsInside(x, y, 52, 130, 56, 20)) {
        Advance();
        return true;
    }

    if (g.stage == 7 && IsInside(x, y, 52, 136, 56, 19)) {
        Advance();
        return true;
    }

    if (g.stage == 5) {
        if (IsInside(x, y, 19, 91, 58, 22)) {
            Advance();
            return true;
        }
    ...
    if (g.stage == 9 && IsInside(x, y, 51, 125, 58, 22)) {
        Advance();
        return true;
    }
